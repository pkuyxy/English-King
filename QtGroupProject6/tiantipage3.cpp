...
TiantiPage3::TiantiPage3(QWidget *parent) : QWidget(parent) {
    // 背景设置
    ...
    BackButton->setStyleSheet(
        ...
        "}");
    connect(BackButton, &QPushButton::clicked, this, &TiantiPage3::backToHome);
    ...
    // 标题
    QLabel *titleLabel = new QLabel("四 级 天 梯 排 行 榜", this);
    ...
    // 调试输出
    qDebug() << "TiantiPage3 initialized with scores:";
    for(int i = 0; i < 5; i++) {
        qDebug() << "Rank" << i+1 << ":" << s[i];
    }
}
...
QString TiantiPage3::getshujukuPath(int difficulty) {
...
void TiantiPage3::loadshujuku(int difficulty) {
...
void TiantiPage3::updateshuju(int newScore) {
    if (newScore > s[4]) {
        s[4] = newScore;
        paixu();
        updateshujuku(Tiantidifficulty);
...
void TiantiPage3::updateshujuku(int difficulty) {
    QFile file(getshujukuPath(difficulty));
