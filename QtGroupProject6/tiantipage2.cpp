...
TiantiPage2::TiantiPage2(QWidget *parent) : QWidget(parent) {
    // 背景设置
    ...
    BackButton->setStyleSheet(
        ...
        "}");
    connect(BackButton, &QPushButton::clicked, this, &TiantiPage2::backToHome);
    ...
    // 标题
    QLabel *titleLabel = new QLabel("高 中 天 梯 排 行 榜", this);
    ...
    // 调试输出
    qDebug() << "TiantiPage2 initialized with scores:";
    for(int i = 0; i < 5; i++) {
        qDebug() << "Rank" << i+1 << ":" << s[i];
    }
}
...
QString TiantiPage2::getshujukuPath(int difficulty) {
...
void TiantiPage2::loadshujuku(int difficulty) {
...
void TiantiPage2::updateshuju(int newScore) {
    if (newScore > s[4]) {
        s[4] = newScore;
        paixu();
        updateshujuku(Tiantidifficulty);
...
void TiantiPage2::updateshujuku(int difficulty) {
    QFile file(getshujukuPath(difficulty));
