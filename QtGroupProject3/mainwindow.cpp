...
if(mode!=4){
    ...
    int yblank=35;
    WordEntry word;
    word = WordLibrary::loadFromFile(getWordLibraryPath(difficulty,mode),mode);
...
void MainWindow::mousePressEvent(QMouseEvent *event)
{
    ...
    if(event->pos().x()>=385&&event->pos().x()<=525&&event->pos().y()>=490&&event->pos().y()<=560){
        end=1-end;
        this->update();
        if(end==1){
            int score2=score;
            ...
            if(currentmode==0){
                round++;
                MainWindow *w2=new MainWindow(currentmode,currentdifficulty,ChineseFont,EnglishFont);
                w2->setWindowTitle("练习模式");
                w2->show();
                this->close();
            }
                ...
                round++;
                MainWindow *w2=new MainWindow(currentmode,currentdifficulty,ChineseFont,EnglishFont);
                w2->setWindowTitle("自定义模式");
                w2->show();
                this->close();
            }
        }
    }
    if(event->pos().x()>=553&&event->pos().x()<=653&&event->pos().y()>=490&&event->pos().y()<=546){
        this->close();
        QString appPath=QApplication::applicationFilePath();
        QProcess::startDetached(appPath,QApplication::arguments());
        QApplication::exit(0);
